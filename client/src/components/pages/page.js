...
function Page(props) {
  ...
  useEffect(() => {
    ...
    if (winter) {
      // will appear only on winter months
      setShowWinter(true);
    }
    if (christmas) {
      // will appear only one week before and after christmas
  ...
  useEffect(() => {
    const storedValidation = localStorage.getItem("isValidate");
    if (storedValidation) {
      setIsValidate(JSON.parse(storedValidation));
    }
  }, []);
  ...
  //       })()}
  //       {(() => {
  //         if ((isEmpty(uuid) && loaded) || (!isEmpty(uuid) && home.loaded)) {
  //           //this is only for Sign or Home
  //           return (
  //             <>
